...
export default function AddNickname() {
  ...
  const [new_nickname, setNewNickname] = useState<{ user: string; value: string }>({
    user: "",
    value: "",
  });
  ...
  const set_nickname = useMutation({
    mutationFn: async () => {
      try {
        const { status, message } = await PATCHRequest(
          "/v1/conversation/" + params.id + "/nicknames",
          {
    ...
    onSuccess: () => {
      query_client.setQueryData<Conversation>(["conversation", params.id], (prev) => {
        ...
        return {
          ...prev,
          nicknames: prev.nicknames.map((nickname) =>
                  ...
                  <UserAvatar
                    src={
                      id === session.user?.id
                        ...
                        : conversation?.members.find((member) => member.id === id)?.photo
                            ?.url
                    }
                    is_online={
                      id === session.user?.id
                        ? false
                        : conversation?.members.find((member) => member.id === id)
                            ?.status === "ONLINE"
                    }
                  />
                  <div className="grid w-full gap-1">
                    <div className="relative">
                      <Input
                        ...
                        onChange={(e) =>
                          setNewNickname({ user: id, value: e.currentTarget.value })
                        }
                      />
                      {!!new_nickname.value && (
                        <Button
                          ...
                          onClick={() =>
                            setNewNickname((prev) => ({ ...prev, value: "" }))
                          }
                        >
                          <X className="h-4 w-auto" />
                        </Button>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="grid text-start">
                    {nickname ? (
                      ...
                      <span className="italic text-muted-foreground w-fit">
                        Set nickname
                      </span>
                    )}
