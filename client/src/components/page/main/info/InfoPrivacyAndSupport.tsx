    ...
    mutationFn: async () => {
      try {
        const { status, message } = await POSTRequest("/v1/conversation/report", {
          ...
          reported_user: conversation?.members.find((member) => member.id !== session!.user.id)?.id,
        });
    ...
    <Collapsible onOpenChange={setOpen}>
      ...
      <CollapsibleContent className="grid gap-2 p-2">
        ...
        {conversation?.is_group_chat ? (
          <Button variant="ghost" className="w-full justify-start" onClick={() => leave_conversation.mutate()}>
