...
if (!client_url) throw new Error("CLIENT_URL is missing from your .env.local file");
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        try {
          const { data, status, message } = await POSTRequest<User>("/v1/user/auth", {
      ...
      try {
        if (profile) {
          const { data } = await GETRequest<User>(
            "/v1/user/@" + profile.email?.substring(0, profile.email.indexOf("@"))
          );
