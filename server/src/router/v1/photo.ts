...
export default function v1PhotoRouter(
  ...
  fastify.delete<{ Body: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!found_photo)
          return reply.code(404).send(JSONResponse("NOT_FOUND", "photo does not exist"));

        if (found_photo.owner.toString() !== user.id)
          return reply
            .code(403)
            .send(JSONResponse("FORBIDDEN", "you are not the owner of this photo"));
